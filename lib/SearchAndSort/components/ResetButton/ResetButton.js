...
export default class ResetButton extends Component {
  ...
  static getDerivedStateFromProps(props, state) {
    // Disable the enter transition if the button is visible on mount
    if (props.visible === true && state.hasEnterTransition === true) {
      return {
  ...
  render() {
    ...
    return (
      <div className={css.resetButtonRoot}>
        <Transition in={visible} timeout={1000} enter={this.state.hasEnterTransition} onExited={this.onExited}>
          {
            (state) => (
